Mapcontrol = function (opt) {
    ...
    // me.div.css("height", ($(window).height() - 160) + "px");
    me.olmapid = uuid();
    var html = '<div id=' + me.olmapid + ' class="map" ></div>';
    me.div.append(html);
//    $("#"+me.olmapid).css("height",($(window).height()-160)+"px");
    me.config = {
        projection: "EPSG:4326"
    };
    me.map = new TMap(me.olmapid, me.config);

    me.map.centerAndZoom(new TLngLat(116.40969, 30.89945), 4);
    //鼠标滚轮缩放
...
//添加缩放条
Mapcontrol.prototype.showZoomCon = function () {
    var me = this;
    var config = {
...
//添加中国境界图层
Mapcontrol.prototype.addChinaLine = function () {
    var me = this;
    var CPlayers = "typhoon:bou2_4p";
    var CPconfig = me.wmsconfig(CPlayers);
    var CLlayers = "typhoon:bou2_4l";
    var CLconfig = me.wmsconfig(CLlayers);
    var url = "http://127.0.0.1:8080/geoserver/typhoon/wms";
    var CPLayer = new TTileLayerWMS("typhoon", url, CPconfig);
    var CLLayer = new TTileLayerWMS("typhoon", url, CLconfig);
    me.map.addLayer(CPLayer);
    me.map.addLayer(CLLayer);
